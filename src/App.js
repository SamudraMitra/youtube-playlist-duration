...
function App() {
  ...
  const fetchVideoIds = async () => {
    while (pageToken !== undefined) {
      ...
      if (res.error) {
        // If the API returns an error, the URL may have a spelling mistake.
        ...
        // Else the API does not return error, push results to array.
  ...
  const handleClick = async () => {
    // User has clicked on Get Length.
    if (playlistUrl) {
      ...
      await Promise.all(arr.map((videoId) => getVideoDetails(videoId, len)));
      if (allDurations.length <= len && len > 0) {
        // The length of the array must be greater than 0 or else the reduce fails.
